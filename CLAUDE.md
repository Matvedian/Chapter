...
| `message_reactions` | `message_id`, `user_id`, `emoji text`. Unique on `(message_id, user_id, emoji)`. REPLICA IDENTITY FULL (needed for DELETE payload in Realtime). RLS: view if in match, insert if in match + own user, delete own. Added to `supabase_realtime` publication. |
| `match_reads` | `(match_id, user_id)` PK, `last_read_at timestamptz`. Tracks when each user last read a match's chat. RLS: view if in match, insert/update own row. Added to `supabase_realtime` publication. |
| `profile_prompts` | `user_id`, `question text`, `answer text`, `position smallint`. Unique on `(user_id, question)`. RLS: readable by any authenticated user, all ops only by owner. Up to 3 per user. |
...
**Book author search — merge order matters:** `searchGoogleBooks` runs two parallel fetches: `q=${query}` (general) and `inauthor:${query}` (author-specific). Results are merged with `byAuthor` first so that books by the searched author appear before books about them. Deduplication is by `external_id`.

**BookDetailModal — description fetch and truncation:** `src/components/BookDetailModal.tsx` fetches description lazily on open. Google Books: `GET /volumes/{id}?key={key}`. Open Library: `GET /works/{key}.json` (description may be a string or `{value: string}`). `stripHtml()` uses `DOMParser` to remove HTML tags. `truncate()` cuts at the last sentence-ending punctuation (`. ! ?`) within 380 chars; falls back to word boundary. The backdrop uses `bg-black/50` — fully transparent fixed overlays don't reliably receive touch events in iOS WebView. Sheet sizes to content (no internal scroll). Wired to: Library grid (cover tap), Library search results (cover tap), Discover profile modal (book row tap — query extended to include `source`/`external_id`), ProfileEdit search + selected books (cover tap), StepBooks onboarding (cover tap).
...
- **Like with comment (2026-06-25):** ❤️ button opens `LikeCommentSheet` (optional 200-char comment). Comment stored on the `swipes` row (requires `comment text` column on `swipes`). Physical right-swipe still records immediately via `onSwipe` — `skipNextSwipeRecord` ref prevents double-insert when the sheet submits.
- **Rewind last swipe (2026-06-25):** `lastSwiped` state stores `{ candidate, direction, matchId }`. Rewind button (between ✕ and ❤️) deletes the swipe row (and match if it was a like), then increments `currentIndex` to re-mount the card. Safety check: `candidates[currentIndex + 1]?.id === candidate.id` prevents stale undos.
- **Emoji reactions (2026-06-25):** `message_reactions` table. Long-press (500ms, cancels on >10px movement) or right-click on a bubble opens a fixed emoji picker (`❤️ 😂 😮 😢 😡 👍`). `toggleReaction` inserts or deletes. Realtime subscribes to INSERT/DELETE on `message_reactions`. Reaction badges grouped by REACTION_EMOJIS order; own reaction has brand-color border.
- **Read receipts (2026-06-25):** `match_reads` table upserted on chat open, on send, and on receiving a partner message. `partnerLastRead` from Realtime `match_reads '*'` subscription. `lastSeenIdx` = last message index where `mine && msg.created_at <= partnerLastRead`; "Seen" shown below that bubble.
- **Relationship goal (2026-06-25):** `profiles.relationship_goal text` (check: `casual`, `serious`, `open`). Required in onboarding StepInfo. Chip selector in ProfileEdit (tap same chip to deselect). Shown as `gender · goal` in Profile, Discover profile modal, and filter sheet. Discover filters exclude candidates whose set goal isn't in the selected filter goals.
- **Profile prompts (2026-06-27):** `profile_prompts` table. `StepPrompts` onboarding step (step 5, optional/skippable) — two-screen: pick up to 3 questions from 12 book-themed options → write answers (150 char limit). `PROMPT_QUESTIONS` exported from `StepPrompts.tsx` and reused in `ProfileEdit`. `candidates.ts` fetches prompts in the existing `Promise.all`. First prompt shown as a callout block on the swipe card. All prompts shown in the Discover profile modal (below favourite books) and on own Profile page. ProfileEdit has inline add/remove/edit with a question picker drawer.
